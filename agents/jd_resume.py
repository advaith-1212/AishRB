...
Usage:
    from jd_resume_agent import generate_resume_for_jd
    result = generate_resume_for_jd(user_id=user_id, jd_text="...", provider="anthropic", api_key="your-key")
"""
...
from services.resume import save_current_resume, get_current_resume, tag_version
from models import get_user_settings, get_user_dir, DATA_DIR
...
def _get_database_path(user_id: int) -> str:
    ...
    if os.path.exists(user_path):
        return user_path
    global_path = os.path.join(_PROJECT_DIR, 'data', 'defaults', 'candidate_database.md')
    return global_path


def _get_rules_path(user_id: int) -> str:
    """Return path to user's resume rules, falling back to global default."""
...
# Default style for PDF rendering -- 10pt, 0.4in margin
DEFAULT_STYLE = {
...
def _extract_yaml_and_assessment(raw: str) -> tuple[str, str]:
    """Split LLM response into YAML and assessment sections."""
    for pattern in [
...
def _validate_yaml(content: str) -> dict:
    """Parse and validate YAML. Raises ValueError on failure."""
    try:
...
def render_pdf(yaml_content: str, user_id: int, style: dict | None = None) -> str:
    """Render resume YAML to PDF. Returns the path to the generated PDF."""
    from jinja2 import Environment, FileSystemLoader
    from weasyprint import HTML
    from models import get_user_dir, get_user_settings, DEFAULT_SECTION_NAMES
...
def count_pdf_pages(pdf_path: str) -> int:
    ...
    with open(pdf_path, 'rb') as f:
        content = f.read()
    # Count page markers in PDF binary -- reliable for WeasyPrint output
    count = content.count(b'/Type /Page')
    # Subtract catalog-level /Pages refs (usually 1)
...
def analyze_jd(jd_text: str) -> dict:
    ...
    blockers = []
    blocker_patterns = {
        'Security clearance required (F-1 visa disqualifies)': [
...
# ---------------------------------------------------------------------------
# Agent 2: Version matcher (local, checks tagged versions)
# ---------------------------------------------------------------------------

def find_reusable_version(user_id: int, jd_text: str, role_type: str) -> tuple[str | None, dict | None]:
    """Find a previously tagged version whose theme tags match this JD.

    Returns (yaml_content, version_meta) or (None, None).

    Matching logic: extract theme tags for the new JD, then find saved versions
    that share the most themes. A version is reusable if it shares at least
    one role-archetype tag (backend, devops, ai-engineer, etc.).
    """
    from models import list_resume_versions, get_resume_version

    # Extract themes for the incoming JD
    ...
    # Require at least 1 overlapping theme tag
...
def extract_jd_tags(jd_text: str, role_type: str) -> list[str]:
    """Extract theme-based tags from a JD for version tagging.

    Tags represent the role archetype and domain themes, NOT individual
    technologies. This lets similar roles reuse the same base resume
    even when the exact tech stack differs slightly.

    Returns 2-5 lowercase theme tags.
    """
    jd_lower = jd_text.lower()

    tags = []

    # Role archetype (primary tag)
    role_themes = {
    ...
    # If no role theme matched, use the detected role_type
    if not tags and role_type != 'general':
        tags.append(role_type)

    # Domain/industry themes
    domain_themes = {
        'fintech': ['fintech', 'financial', 'banking', 'payments', 'trading', 'capital markets', 'investment'],
        'healthcare': ['healthcare', 'health tech', 'medical', 'clinical', 'hipaa', 'biotech'],
        'enterprise': ['enterprise', 'b2b', 'saas platform', 'multi-tenant'],
        'startup': ['startup', 'early-stage', 'series a', 'series b', 'seed stage', 'fast-paced'],
        'observability': ['observability', 'monitoring', 'reliability', 'incident', 'on-call'],
        'data-platform': ['data mesh', 'data catalog', 'data governance', 'data product', 'data lakehouse'],
        'developer-tools': ['developer tools', 'developer experience', 'dx', 'internal tools', 'platform team'],
        'cloud-native': ['cloud-native', 'cloud native', 'containerized', 'serverless', 'microservices architecture'],
    }
...
Experience Gaps: [missing skills -- NOT years of experience; +-2 years is fine]
Blockers: [ONLY hard blockers: visa sponsorship denial, US citizenship required, security clearance.
           Experience years +-1-2 is NOT a blocker. Company type (startup vs FANG) is NOT a blocker.]
"""


def _build_yaml_template(user_id: int) -> str:
    """Build a YAML structure template from the user's settings and current resume."""
    settings = get_user_settings(user_id)
    header = settings.get('header', {})
    name = header.get('name', '[Your Name]')
    contact = header.get('contact', {})

    # Try to get structure from existing resume
    ...
    # Use existing resume structure for education if available
    ...
    # Use existing experience structure if available
    if existing.get('experience'):
        ...
        for exp in existing['experience']:
            exp_template.append({
                ...
                'bullets': ['[JD-relevant bullets, bold action phrase with **, wrap in double quotes]'],
            })
...
def _get_generate_system(user_id: int) -> str:
    """Build the generator system prompt with a user-specific YAML template."""
...
- Missing a required skill listed in "Requirements" costs 5-10 points
- Generic phrasing without JD keywords costs 3-5 points per instance
- Return ONLY JSON, no markdown, no prose
"""

_COMPRESS_SYSTEM = """\
You are a resume density optimizer. The resume YAML is currently rendering to MORE THAN 1 PAGE.
You must reduce its length to fit exactly 1 page while preserving all important content.
...
3. Combine related bullets using semicolons: "Built X; reduced Y by 40%"
4. Shorten verbose phrases: "designed and implemented" -> "Built"
5. Remove the least relevant bullets (lowest JD match) from experience/projects
6. Reduce project bullets from 5 to 3 per project
7. Combine skill categories if too many (max 4-5 categories)
...
- Keep all double-quote wrapping on bullets with special chars
- Return ONLY valid YAML, no fences, no prose
- The resume MUST be shorter than the input

You will receive the current YAML and the page count. Cut intelligently.
"""
...
- Use abbreviations (env, infra, k8s, CI/CD, etc.) to save space
- Bold action phrases with ** markers
- Wrap bullets with special chars in double quotes
...
def _generate_resume(
    ...
    """Generate a tailored resume YAML from JD + candidate database.

    If base_yaml is provided, uses it as a starting point (version reuse).
    Returns (yaml_content, assessment).
    """
    database = _read_file(_get_database_path(user_id))
    rules = _read_file(_get_rules_path(user_id))

    # Include learnings from past user edits
    ...
    if learnings:
        parts.append(
            f"LEARNINGS FROM PAST USER EDITS (apply these patterns to this resume):\n{learnings}"
        )
    if base_yaml:
        parts.append(
            f"BASE RESUME (from a similar role, refine and adapt for this specific JD):\n{base_yaml}"
        )
...
def _check_and_fit_pdf(
    ...
    max_compress_rounds: int = 2,
) -> tuple[str, int]:
    """Render PDF, check page count, compress if >1 page.

    Returns (yaml_content, page_count).
    """
    style = style or DEFAULT_STYLE.copy()
    ...
    for attempt in range(max_compress_rounds + 1):
        ...
        try:
            ...
            break  # Keep current version if compression broke YAML
...
# ---------------------------------------------------------------------------
# Agent 5: ATS Verifier (separate from generator)
# ---------------------------------------------------------------------------
...
def score_resume_ats(
    ...
    """Independent ATS scoring of a resume against a JD."""
...
# ---------------------------------------------------------------------------
# Agent 6: Improver (applies ATS feedback)
# ---------------------------------------------------------------------------
...
def _improve_resume(
    ...
    """Apply ATS feedback to improve resume while keeping it compact."""
    database = _read_file(_get_database_path(user_id))
...
def generate_resume_for_jd(
    ...
    Returns dict with keys:
        yaml, version_id, ats_score, ats_details, assessment, pages,
        role_type, blockers, tags, iterations, logs
    """
    style = style or DEFAULT_STYLE.copy()
    logs = []
    ...
    for i in range(max_iterations):
        ...
        # Phase 6: Improve based on ATS feedback
    ...
    # Final PDF render to ensure preview is current
...
# ---------------------------------------------------------------------------
# Learning system: diff agent output vs user edits, store patterns
# ---------------------------------------------------------------------------

def _get_learnings_path(user_id: int) -> str:
    """Return path to user's resume learnings file."""
    return os.path.join(get_user_dir(user_id), 'resume_learnings.md')


def diff_versions(agent_yaml: str, user_yaml: str) -> list[dict]:
    """Compare agent-generated YAML vs user-edited YAML.

    Returns a list of changes: [{section, type, agent_value, user_value}, ...]
    """
    agent = yaml.safe_load(agent_yaml) or {}
    user = yaml.safe_load(user_yaml) or {}
    changes = []

    # Compare top-level string fields
    for key in ('summary',):
        av = agent.get(key, '')
        uv = user.get(key, '')
...
def _item_name(item: dict) -> str:
    """Extract a readable name from an experience/project item."""
    return item.get('company') or item.get('name') or item.get('institution') or '?'


def save_learning(user_id: int, tags: list[str], changes: list[dict], reason: str) -> None:
    """Append a learning entry to the user's resume learnings file.

    Each entry records: what theme tags, what the user changed, and why.
    The agent reads this file on future generations to repeat good patterns.
    """
    ...
    for c in changes:
        ...
        if ctype == 'bullets_changed':
            ...
            entry_lines.append(f"- `{section}`: {len(removed)} bullet(s) removed, {len(added)} added")
            for b in added[:3]:
                entry_lines.append(f"  + {b[:120]}")
            for b in removed[:3]:
                entry_lines.append(f"  - {b[:120]}")
        elif ctype == 'reordered':
            entry_lines.append(f"- `{section}`: reordered from {c.get('agent_order')} to {c.get('user_order')}")
            ...
            entry_lines.append(f"- `{section}`: categories modified")
        elif ctype == 'rewritten':
            entry_lines.append(f"- `{section}`: rewritten")
        else:
    ...
    if os.path.exists(learnings_path):
        ...
        with open(learnings_path, 'w', encoding='utf-8') as f:
            ...
            f.write("The agent reads this file to improve future generations.\n")
            f.write(entry)


def get_learnings(user_id: int) -> str:
    """Read the user's learnings file, or return empty string if none exist."""
    learnings_path = _get_learnings_path(user_id)
    if os.path.exists(learnings_path):
...
def get_last_agent_version(user_id: int) -> dict | None:
    """Get the most recent agent-generated version for diffing."""
    from models import list_resume_versions, get_resume_version
...
if __name__ == '__main__':
    ...
    parser.add_argument('--jd', type=str, help='Job description text (or path to .txt file)')
    parser.add_argument('--provider', type=str, default='anthropic',
                        choices=['anthropic', 'openai', 'gemini'])
    parser.add_argument('--api-key', type=str, help='LLM API key (or set via env var)')
    parser.add_argument('--model', type=str, default=None)
    parser.add_argument('--user-id', type=int, default=2)
    parser.add_argument('--target-score', type=int, default=90)
    ...
    jd_text = args.jd or ''
    if jd_text and os.path.isfile(jd_text):
        jd_text = _read_file(jd_text)
    if not jd_text:
        print('Error: --jd required')
        raise SystemExit(1)
